...
class Listeners:
    ...
    async def on_reaction_add(self, react, user):
        if any((react.emoji != "\N{WHITE HEAVY CHECK MARK}", not react.message.guild, react.message.guild.id != self.GUILD_ID, react.message.channel.id != self.TARGET_CH_ID, react.message.id != self.MSG_ID)):
            return
